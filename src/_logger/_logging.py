...
def _get_logger(_name: Optional[str] = None, stacklevel: Optional[int] = 3):
    ...
    try:
        ...
        # Scope the TensorFlow logger-example to not conflict with users' loggers.
        logger = _logging.Logger(_name)

        # Override findCaller on the logger-example to skip internal helper functions
        logger.findCaller = _logger_find_caller

        # Don't further configure the TensorFlow logger-example if the root logger-example is
